...
/**编写程序实现从键盘终端输入语法正确的算术表达式，计算出表达式的值。
 （1）为了避免算符的二义性，可以假设表达式中只包含实型正常数；
 （2）运算符包括加、减、乘、除四种基本运算，可以包含圆括号。
 */
        ...
        public void getExpressionValue(String A){
            ...
            for(int i = 0;i < A.length();i++){
                ...
                if(arrayA[i] >= '0' && arrayA[i] <= '9'){
                    ...
                    i--;
                    //对应上面一句i = i+1;因为在for循环中有i++自增操作，若不执行此句，会导致i自增两次
                    Value.push(temp);
                }
                else{
                    if(Operator.empty()){
                        ...
                        if(judge == -1){
                            //此时，说明当前栈顶字符为')'，这是不存在的，因为遇到')'，按要求不让进栈
